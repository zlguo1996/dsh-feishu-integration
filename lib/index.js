...
export function apply(ctx, config = {}) {
  const {
    ...
    notificationFormat = 'card',    // 'card'=渐进披露卡片（结论行+要点+折叠正文）| 'text'=旧纯文本
    maxDetailChars = 6000,          // 折叠正文上限：折叠态不占聊天列表，可比 maxText 宽松
  ...
  const summary = installSummaryPush(ctx, {
    ...
    notificationFormat, maxDetailChars, foldThresholdChars,
  ...
  if (takeoverInbound && answerFromFeishu !== false) {
    // ctx 日志不落文件（只在 GUI 控制台），提问桥排障需要持久痕迹
