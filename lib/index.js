 ...
 *   未命中 → 按 defaultSessionPolicy 处理（默认 'fresh'：每条消息新建会话，
 *   避免固定会话上下文无限膨胀；'fixed'/'idle' 见 index.js 配置注释）。
 *   设置页 UI 走 /feishu connection RPC。
 *
...
import { createDigestHistoryStore } from './host/digest-history-store.js'
...
export function apply(ctx, config = {}) {
  const {
    // ⚠️ 已废弃：卡片头部标题不再是固定文案，而是由 formatter 按「本回合提问的主题」
    // 产出（≤20 字）；取不到来源时用结构哨兵「本回合回复」。该项保留仅为兼容旧
    // profile，**不再影响渲染**。
    title = 'dsh 回复总结',
    maxText = 1500,                 // 仅「线程不可用」降级时，纯文本里附带全文的上限
    notificationFormat = 'card',    // 'card'=话题根卡片（标题+摘要+要点+元信息）| 'text'=纯文本根消息
    foldThresholdChars = 400,       // 仅降级路径用：短于该长度不折叠
    notificationFormatter = {},     // 摘要用的发送层 LLM：{enabled, provider, model, timeoutMs, maxTokens}
    includeReasons = ['completed'],
    openId = '',
    chatId = '',
    botSelection = 'all',
    takeoverInbound = false,        // true=启动长连接接收事件
    harnessBaseUrl = '',            // 缺省用 ctx.webServer.port 自动推导
    workspace = process.cwd(),      // 创建新固定会话时的工作区路径
    agentPreset = 'standard',
    // 未引用会话时的默认会话策略：'fresh'=每条消息都开新会话（默认，避免固定会话
    // 上下文无限膨胀）| 'fixed'=复用同一个固定会话（旧行为）| 'idle'=空闲超时才换新。
    defaultSessionPolicy = 'fresh',
    defaultSessionIdleMinutes = 30, // 仅 'idle' 用：空闲超过该分钟数则下次新建会话
    replyMapMax = 500,              // reply-map LRU 上限
    replyMapTtlDays = 7,            // 映射有效期（digest-history 复用同一 TTL）
    digestHistoryMaxTurns = 20,     // 每个 session 保留的摘要历史回合数（formatter 背景用）
    threadDelivery = true,          // true=完整回复以飞书原生话题线程回复承接
    replyTimeoutMs = 600000,        // 已废弃：入站路径不再等待回合，此值已无读取方（保留键位，避免旧配置校验失败）
    replyMaxChars = 9000,           // 单条回帖/线程分段上限
  ...
  // 出站摘要历史：后续回合要拿「最近 5 条（用户输入 + 当时的总结）」当 formatter 背景，
  // 而总结文本此前不存在于任何持久化状态里 —— 单独一份，且**先落盘再投递**。
  const digestHistoryStore = createDigestHistoryStore({
    historyPath: join(feishuDir, 'digest-history.json'),
    maxTurnsPerSession: digestHistoryMaxTurns,
    ttlDays: replyMapTtlDays,
    log,
  })
  const feishuApi = createFeishuApi()
  // ── 持久化调试日志（一份文件，出站 formatter / 提问桥 / 卡片作答共用）──
  // 为什么需要它：插件的 ctx 日志只进 GUI 控制台、**不落盘**（下面 log 走 ctx.logger），
  // 于是「摘要到底走没走 LLM」「提问桥为什么没发帖」在磁盘上都不可观测 ——
  // 排障只能靠肉眼盯控制台，且「没问题」与「根本没跑」长得一模一样。
  // 这里把关键事件追加到 <dshHome>/integrations/dsh-feishu/bridge-debug.log。
  // 失败绝不影响主流程（整体 try/catch 吞掉）。
  const debugLogPath = join(feishuDir, 'bridge-debug.log')
  const debugLog = (...args) => {
    try {
      mkdirSync(dirname(debugLogPath), { recursive: true })
      appendFileSync(debugLogPath, `[${new Date().toISOString()}] ${args.join(' ')}\n`)
    } catch { /* 调试日志失败不影响主流程 */ }
  }
  ...
  // ── 飞书回合的「表情翻转」登记表 ──
  // 入站路径注入 prompt 时登记一个翻转闭包；回合结束时由 summary-service 的 turn/end
  // 处理器（与出站投递同一个 handler、同一个事件源）取出执行。表情因此不再依赖入站
  // 路径等待回合，replyTimeoutMs 那条硬超时路径彻底没有调用者。
  // 按 sessionId 入队：fresh 策略下一个会话只有一条；fixed/idle 复用时按 FIFO 对应。
  const pendingReactions = new Map()
  const registerPendingReaction = (sessionId, entry) => {
    if (!sessionId || !entry) return
    const queue = pendingReactions.get(sessionId) ?? []
    queue.push(entry)
    pendingReactions.set(sessionId, queue)
  }
  const takePendingReaction = (sessionId) => {
    const queue = pendingReactions.get(sessionId)
    if (!queue?.length) return null
    const entry = queue.shift()
    if (queue.length === 0) pendingReactions.delete(sessionId)
    return entry
  }

  // ── 出站：总结推送 + 记账 + 防回环 ──
  const summary = installSummaryPush(ctx, {
    maxText, includeReasons, botSelection, openId, chatId,
    ...
    // 线程回复用**卡片**：飞书纯文本消息不渲染 Markdown（`**加粗**` 会显示成星号），
    // 只有卡片的 markdown 组件会渲染；replyTextMessage 仅作老客户端降级。
    replyCardMessage: feishuApi.replyCardMessage,
    replyTextMessage: feishuApi.replyToMessage,
    readBotState: botStore.readBotState,
    recordReplyMapping: replyMapStore.record,
    recordDigest: digestHistoryStore.record,
    readDigestHistory: digestHistoryStore.getForSession,
  ...
  if (takeoverInbound && answerFromFeishu !== false) {
    ...
    questionSeam = installHostQuestionSeam(ctx, questionBridge, { log, debug: debugLog })
  }
  ...
  function launchBot(bot, appSecret) {
    ...
    record.starting = startInboundForBot({
      ...
      defaultSessionPolicy, defaultSessionIdleMinutes,
      ...
      helpers: {
        ...
        // 回合结束时翻表情：由共享的 turn/end 处理器取出执行（事件驱动，无等待）
