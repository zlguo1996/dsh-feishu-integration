...
export function installSummaryPush(ctx, deps) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'turn/end') {
      ...
      // 不存在超时把表情（或回复）丢在半路的问题。放在 reason 白名单之前，是为了让未完成
      // 的回合也能把表情收掉，不留停住的 OnIt（与旧行为「ask() 一返回就翻 DONE」对齐）。
      try {
        const pending = takePendingReaction?.(sid)
        if (pending?.flip) {
          Promise.resolve(pending.flip('DONE')).catch((err) => log('warn', '[表情] 翻转失败:', String(err)))
        }
      } catch (err) {
