...
export async function startInboundForBot(opts) {
  ...
  const { log, debugLog = null, handleCardAction = null, registerPendingReaction = null } = helpers
  ...
  const { rpc, sessionExistsSafe, createFixedSession, injectPrompt } = gateway
  ...
  async function handle(event, mapped) {
    ...
    try {
      // 只注入，**不等回合**。出站投递与「回合结束」都挂在 session/event 的 turn/end 上
      // （与 Web 发起的回合同一条路径），所以这里不再需要等待，也就没有超时可言 ——
      // 原先 await ask() 的 10 分钟硬超时会**静默丢弃**回复，正是这个 bug 的根源。
      await injectPrompt(sessionId, text)
    } catch (err) {
      log('warn', '注入 prompt 失败:', String(err))
    ...
    // 「回合结束时把 OnIt 翻成 DONE」交给共享的 turn/end 处理器执行：这里只登记一个
    // 闭包，由它取出调用。于是表情不再依赖任何等待/轮询，也就不会再出现「表情永远停在
    // OnIt」的残留（长回合尤甚）。注册表里存的是函数，不携带任何凭据。
    registerPendingReaction?.(sessionId, {
      messageId,
      flip: (emojiType) => finishReaction(messageId, reaction, emojiType),
    })
  }
