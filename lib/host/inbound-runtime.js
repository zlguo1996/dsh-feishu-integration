...
export async function startInboundForBot(opts) {
  ...
  const dispatcher = new Lark.EventDispatcher({}).register({
    ...
    // 卡片回传交互（卡片上的按钮/下拉被点击）。当前为**链路探针**：记录回调到
    // card-action-probe.log、打一条 warn 日志，然后回一个 toast。不参与作答、
    // 不改卡片内容。
    //
    // 为什么必须**注册**这个 handler（而不只是不处理）：没有 handler 时，
    // Lark SDK 的 EventDispatcher 会 warn `no <type> handle` 并把它当作回调响应
    // 原样返回字符串 `no card.action.trigger event handle`；平台无法把这段字符串
    // 解析成合法的回调响应，客户端就报 **200672**。所以「收到回调」与「注册了
    // handler」是两件事：前者已实测成立，后者此前缺失才导致报错。
    // 回 toast 而不是空对象：让「回调真的往返成功」对用户肉眼可见。
    // 必须同步返回：飞书要求 3 秒内响应回调。
    'card.action.trigger': (event) => {
      ...
      const label = seen.value?.option ?? seen.value?.kind ?? seen.tag ?? '未知'
      return { toast: { type: 'info', content: '✅ 探针已收到点击：' + label } }
    },
  })
