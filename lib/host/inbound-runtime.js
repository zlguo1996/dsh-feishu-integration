...
export async function startInboundForBot(opts) {
  ...
  const dispatcher = new Lark.EventDispatcher({}).register({
    ...
    // 卡片回传交互（卡片上的按钮/下拉被点击）。当前仍是**链路探针**：只服务
    // 固定的测试卡片（两个选项），不参与真实提问/作答；但已按官方「方式一：
    // 3 秒内立即更新卡片」返回更新后的卡片，用于验证"点击即变选中态"。
    //
    // 为什么必须**注册**这个 handler（而不只是不处理）：没有 handler 时，
    // Lark SDK 的 EventDispatcher 会 warn `no <type> handle`，并把这个警告字符串
    // 当作回调响应原样返回（`no card.action.trigger event handle`）；平台无法解析
    // 成合法回调响应，客户端就报 **200672**。所以「收到回调」与「注册了 handler」
    // 是两件事：前者已实测成立，后者此前缺失才导致报错。
    //
    // 响应体（官方结构）：{ toast, card: { type: 'raw' | 'template', data } }
    //   - raw      → data 是完整卡片 JSON（本处使用）
    //   - template → data 是 { template_id, template_version_name?, template_variable? }
    // 只回 toast 或 `{}` 即「方式二：不更新卡片」。
    // 硬约束：必须 3 秒内以 HTTP 200 响应；不可用 3xx 响应回调。
    'card.action.trigger': (event) => {
      ...
      const { option: chosen = null, kind = 'answer' } = seen.value ?? {}
      const cancelled = kind === 'cancel'
      const elements = [{ tag: 'markdown', content: '**打包要不要带上测试环境？**' }]
      for (const label of ['带上测试环境', '不带']) {
        const selected = label === chosen
        elements.push({
          tag: 'button',
          element_id: label === '带上测试环境' ? 'opt_yes' : 'opt_no',
          type: selected ? 'primary_filled' : 'default',
          text: { tag: 'plain_text', content: (selected ? '✓ ' : '') + label },
          disabled: true,
          behaviors: [{ type: 'callback', value: { kind: 'answer', option: label, rpcId: 'probe-A' } }],
        })
      }
      // 「取消」在已答态下已无意义，但**必须保留**：一是元素数量/位置在状态切换
      // 前后保持一致（否则用户会觉得「选项少了一个」），二是 element_id 集合稳定
      // 才能支撑将来的 CardKit 元素级更新。
      elements.push({
        tag: 'button',
        element_id: 'opt_cancel',
        type: 'default',
        text: { tag: 'plain_text', content: '取消本次提问' },
        disabled: true,
        behaviors: [{ type: 'callback', value: { kind: 'cancel', rpcId: 'probe-A' } }],
      })
      elements.push({
        tag: 'markdown',
        content: cancelled ? '⛔ 已取消本次提问' : '✅ **已选择：' + String(chosen) + '**',
      })
      return {
        toast: {
          type: cancelled ? 'warning' : 'success',
          content: cancelled ? '已取消' : '已选择：' + String(chosen),
        },
        card: {
          type: 'raw',
          data: {
            ...
            config: {
              update_multi: true,
              summary: { content: 'DSH 提问（' + (cancelled ? '已取消' : '已回答：' + String(chosen)) + '）' },
            },
            header: {
              template: cancelled ? 'grey' : 'green',
              title: { tag: 'plain_text', content: 'DSH 提问 · ' + (cancelled ? '已取消' : '已选择') },
            },
            body: { elements },
          },
        },
