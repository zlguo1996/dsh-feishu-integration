...
import { findReplyMapping } from '../shared/reply-routing.js'
...
export async function startInboundForBot(opts) {
  ...
  const { log, debugLog = null, handleCardAction = null, deliverTurnNotification = null } = helpers
  ...
  // sessionRouteInfo 不再取用：它原本只服务于「已转发到哪个会话」那条回执，回执已按用户要求去掉。
  const { rpc, sessionExistsSafe, createFixedSession, ask } = gateway
  ...
  async function handle(event, mapped) {
    ...
    // 不再回「✅ 已转发到对应 DSH 会话」那条回执。
    // 为什么去掉：`OnIt` 表情本身已经表达了「收到、正在处理」，再补一条消息只是刷屏；
    // 用户 2026-09-24 明确要求去掉。需要「这轮进了哪个会话」时看上面那行 info 日志。
    // 注意：回执原先也承担「可长按引用」的锚点，但入站消息本身与最终回答都仍写
    // reply-map，所以引用续聊的能力不受影响。
    const reaction = await addReactionSafe(messageId, 'OnIt')

    try {
      const { text: answer, turn: answerTurn } = await ask(sessionId, text)
      await finishReaction(messageId, reaction, 'DONE')
      // 卡片即投递：飞书发起的回合改走总结卡片（摘要卡 + 话题线程里的全文），纯文本回帖
      // 只在卡片**没有完整送达**时兜底。三个理由：
      // - 纯文本走 msg_type:'text'，飞书不渲染 Markdown（`**加粗**` 会显示成星号）；卡片
      //   的话题线程走 markdown 组件，正文可读性正常；
      // - 用户明确要的是「和 Web 发起的回合一样的卡片」；
      // - 二者只能二选一，否则同一个回合的正文会投递两遍。
      let cardDelivered = false
      if (deliverTurnNotification && answerTurn != null) {
        try {
          const res = await deliverTurnNotification({
            sessionId, turn: answerTurn, replyOverride: answer || null, questionOverride: text,
          })
          cardDelivered = !!res?.delivered
          if (!cardDelivered) {
            log('warn', `[投递] 卡片未完整送达(${res?.reason ?? 'unknown'})，回退纯文本回帖`)
          }
        } catch (err) {
          log('warn', '[投递] 卡片投递异常，回退纯文本回帖:', String(err))
        }
      }
      if (!cardDelivered) {
        // 兜底路径：内容宁可重复投递（根卡片可能已发出），也不能丢。
        await replyToSession(messageId, answer || '（该回合结束但没有文本回复）', sessionId, mapped?.turn)
      }
    } catch (err) {
      log('warn', '处理回复失败:', String(err))
      // 等待回答超时：飞书侧保持静默（保留 OnIt，不追加错误反馈）。
      // ⚠️ 去掉转发回执后，超时情况下飞书侧**只剩那个 OnIt 表情**、没有任何文字。
      // 这是刻意的（不用错误文本刷屏），代价是用户只能从表情判断状态。
