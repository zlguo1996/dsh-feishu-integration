...
export function createNotificationFormatter({ ctx, config = {}, log } = {}) {
  ...
  /**
   * 模型路由：显式配置优先；否则用 DSH 的 agent 默认模型（`ctx.agentDefaultModel`），
   * 于是零配置即可工作且**不写死任何 provider**。每次调用时解析，设置页改了默认
   * 模型立即生效。两者都拿不到 → 不做 LLM，直接用确定性兜底。
   */
  ...
  async function format(reply, opts = {}) {
    ...
    try {
      ...
      const raw = await Promise.race([collect(reply, opts.sessionId, controller.signal, route), deadline])
