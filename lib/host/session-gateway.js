...
export function createSessionGateway({
   ...
   * 轮询 session.history 直到该回合 turn/end。
   *
   * 返回 `{text, turn}`：`text` 是累计 assistant 文本，`turn` 是**被本条 prompt 锚定的
   * 回合号**。调用方（入站路径）要用 turn 去手动触发出站总结投递，所以必须一并返回，
   * 不能只给文本。
   */
  async function ask(sessionId, text) {
    ...
    return { text: latestText.trim(), turn: targetTurn }
  }
