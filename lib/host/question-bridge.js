...
export function createQuestionBridge({
  ...
  async function applyParsedAnswer(hit, parsed, { messageId = null, notify = null, record = false } = {}) {
    ...
    // ⚠️ `answeredByUs` 必须在**发起提交之前**置位。宿主接缝（host-question-seam）
    // 在 respond 里**同步**注入 `question/resolved`；若等提交返回再置位，
    // handleResolved 读到的仍是 false，就会把**我方自己的作答**误判成「网页端先答」，
    // 往线程里补一条「ℹ️ 该问题已在 DSH 网页端被回答，会话继续执行中。」——
    // 2026-09-24 线上报障正是如此（旧 mux 路径的回帧是异步的，所以那时不会触发）。
    // 取消路径（interceptReply）一直是提前置位的，这里与之对齐。
    ...
    // answeredByUs 已在**发起提交之前**置位（见上面那段的说明），此处不再重复。
    clearBatch(hit)
    if (record && messageId) {
  ...
  function handleResolved(payload) {
    ...
    // 卡片刷新**不分来源**：文本回帖作答没有卡片回传可以承载结果，只能靠这次
    // PATCH 把卡片落到已答态，所以「我方作答」时也必须刷。
    if (outcome !== 'cancelled') {
      // 网页端赢：卡片还停在待答态，刷成已答态（卡片本身就是回执）；
      // 我方作答：补一次 PATCH，让卡片与作答结果一致。
      void settleCardFromWebAnswer(seen, { sessionId, answer })
        .catch((err) => log('warn', '刷新提问卡片失败:', String(err)))
    }
    // 但「已在网页端被回答」这条说明**只**能在网页端真的抢答时才发。
    // answeredByUs 由 applyParsedAnswer 在发起提交**之前**置位——宿主接缝是在
    // respond 内**同步**注入本帧的，置位晚一拍就会把我方作答误报成网页端抢答
    // （2026-09-24 线上报障）。
