 ...
 *
 * `metaExtra`：附加到元信息行的短标注（例如「正文取自推理通道」）。默认为空，
 * 因此老调用方的输出逐字不变。
 */
export function buildNotificationText({ title = '', turn, summary = '', bullets = [], cwd = '', metaExtra = '' } = {}) {
 ...
 * @param {string} [o.metaExtra]  附加到元信息行的短标注（如「正文取自推理通道」）；默认为空
 */
export function buildNotificationCardV2({
  ...
  // 元信息只保留 turn 与 cwd（外加调用方显式给的 provenance 标注）：
  // 不带标题、不带任何固定文案。
  const meta = [
    turn != null && turn !== '' ? 'turn ' + turn : '',
    cwd ? 'cwd: ' + cwd : '',
