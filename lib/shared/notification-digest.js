 ...
 * 通知摘要的语义层契约与极简确定性兜底。
 *
 * 分工（Codex 第 5 轮 FINAL，用户已确认）：
 * - **语义交给 LLM**：它只做「受约束压缩」，输出 {summary, bullets} 纯文本 JSON；
 * - **确定性代码只做两件事**：显然正确的兜底 + 契约校验。
 *
...
import { parseBlocks, stripInline, firstSentence, clip } from './progressive.js'
 ...
 * @returns {{summary: string, bullets: string[]} | null}
 */
 ...
 * @returns {{ok: true, digest: {summary: string, bullets: string[]}} | {ok: false, reason: string, violations?: string[]}}
 */
export function validateDigest(candidate, source) {
  ...
  const violations = coverageViolations(joined, source)
  if (violations.length > 0) return { ok: false, reason: 'source-coverage', violations }
  return { ok: true, digest: { summary: summary.trim(), bullets: bullets.map((b) => b.trim()) } }
}
